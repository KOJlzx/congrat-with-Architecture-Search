...
class AdvancedGraphModelTrainer:
    def __init__(self, config: TrainingConfig, worker_id: int, rank: int, world_size: int):
        self.config = config
        ...
        self.k_hop = self.model_config.model.init_args.gnn_params.num_layers
        # 初始化数据模块
        self._setup_data(rank, world_size)
    ...
    def _setup_data(self, rank: int, world_size: int):
        """设置数据模块"""
        # 创建数据模块
        datamodule = PubmedGraphTextDataModule(**self.model_config.data.init_args)
        datamodule.prepare_data()
        datamodule.setup(rank, world_size, self.k_hop)
        ...
        self.train_dataloader = DataLoader(
            ...
            shuffle=True,
            num_workers=1,
            pin_memory=self.model_config.data.init_args.pin_memory,
...
def train_distributed_advanced(config: TrainingConfig):
    ...
    # 创建多个训练器，每个使用对应的数据分片
    trainers = [
        AdvancedGraphModelTrainer.remote(config, i, i, config.num_workers) 
